...
export const ListItem: React.FunctionComponent<Props> = (props) => {
    ...
    return (
        <div key={id} style={{marginBottom: "10%"}}>
            ...
            <div>内容: {content}</div>
            <Button variant="outlined" color="error" style={{ marginTop: "10px", marginRight: "10px" }} onClick={() => { removeTodo(id); }}>削除</Button>
            <Button variant="outlined" style={{ marginTop: "10px" }} onClick={() => { handleEditButtonPushed(id, content); }}>編集</Button>
        </div>
    )
}
